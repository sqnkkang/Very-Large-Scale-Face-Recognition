...
class MultiLMDBDataset(Dataset):
    def __init__(self, source_lmdbs, source_files, feat_lmdbs=None, feat_files=None, transforms=None, return_feats=False):
        # 判断传进来的两个地址是不是 list 或者 tuple 类型，不是的话转化为一个氮元素的列表
        ...
        # 检查这两个列表里面的元素个数是否相等（必须有元素并且元素的个数相同），不相等的话抛出异常，代表我们做错了
        ...
        max_label = 0
        last_label = 0
        # 打开很多个数据库，max_label 和 last_label 的作用的记录当前的数据库的最后一个label然后下一个数据库的文件读取编号的时候从当一个位置的下一个位置开始避免重复编号
        # 适用于多数据库的情况
        for db_id, file_path in enumerate(source_files):
            with open(file_path, 'r') as fin:
                for line in fin:
                    ...
                    if len(l) > 0:
                        items = l.split(' ')
                        print(items[0], items[1], db_id, last_label)
...
if __name__ == '__main__':
    ...
    dataset = PairLMDBDatasetV2(source_lmdb, source_file)
